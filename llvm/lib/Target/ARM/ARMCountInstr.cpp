...
#define DEBUG_TYPE "arm-count-instr"

static cl::opt<bool>
  EnableCountInstructions("enable-arm-count-instr",
            cl::desc("Enable the ARM count instr."),
            cl::init(false), cl::Hidden);

namespace {
class ARMCountInstructions : public MachineFunctionPass {
public:
  ARMCountInstructions() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override { return "ARMCountInstr"; }
...
char ARMCountInstructions::ID = 0;

bool ARMCountInstructions::runOnMachineFunction(MachineFunction &MF) {
...
bool ARMCountInstructions::isCmovInstruction(const MachineInstr &MI) const {
  auto opcode = MI.getOpcode();
  if (opcode == ARM::MOVCCr || opcode == ARM::MOVCCi) {
    return true;
  }
  return false;
}

void ARMCountInstructions::getAnalysisUsage(AnalysisUsage &AU) const {
  MachineFunctionPass::getAnalysisUsage(AU);
}

INITIALIZE_PASS(ARMCountInstructions, DEBUG_TYPE,
                "Count Conditional Move Instructions", false, false)

FunctionPass *llvm::createARMCountInstructionsPass() {
  return new ARMCountInstructions();
}
