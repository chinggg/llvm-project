...
#define DEBUG_TYPE "riscv-count-instr"

static cl::opt<bool>
  EnableCountInstructions("enable-riscv-count-instr",
            cl::desc("Enable the RISC-V count instr."),
            cl::init(false), cl::Hidden);

namespace {
class RISCVCountInstructions : public MachineFunctionPass {
public:
  RISCVCountInstructions() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override { return "RISCVCountInstr"; }
...
char RISCVCountInstructions::ID = 0;

bool RISCVCountInstructions::runOnMachineFunction(MachineFunction &MF) {
...
bool RISCVCountInstructions::isCmovInstruction(const MachineInstr &MI) const {
  // Check for RISC-V select instructions which implement conditional moves
  // NOTE: not sure if these are really "conditional move" in RISCV
  switch (MI.getOpcode()) {
...
void RISCVCountInstructions::getAnalysisUsage(AnalysisUsage &AU) const {
  MachineFunctionPass::getAnalysisUsage(AU);
}

INITIALIZE_PASS(RISCVCountInstructions, DEBUG_TYPE,
                "Count Conditional Move Instructions", false, false)

FunctionPass *llvm::createRISCVCountInstructionsPass() {
  return new RISCVCountInstructions();
}
